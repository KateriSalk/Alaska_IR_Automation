...
The required packages for the *data_pull.R* code are [*EPATADA*](https://github.com/USEPA/EPATADA) and [*tidyverse*](https://www.tidyverse.org/). To install the *EPATADA* package, you will also need the [*remotes*](https://cran.r-project.org/web/packages/remotes/index.html) package. Note, the *remotes* package is not required beyond installation of the *EPATADA* package. Below is a code chunk that demonstrates how to install and load these packages.
...
The inputs into the *TADA_BigDataRetrieval* are the dates defined in the previous section, the WQP site type, the state code (i.e., AK), and applying auto clean. Note, at the time of development, the *TADA_BigDataRetrieval* function will not return a 'TADA object' without the applyautoclean = T.
